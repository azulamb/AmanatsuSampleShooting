...
public class OP extends GameView
{
  ...
  public void UserInit()
  {
    ...
    // 画像が正方形でない上に、res内にあるため、変な拡大をされる可能性がある。
  ...
  public boolean MainLoop()
  {
    ...
    if( timer < 0 )
    {
      ...
      // ロゴのアルファ値を少しずつ下げていく。
      draw.setAlpha( R.drawable.title, Math.abs( (float)timer ) / 60.0f );
    ...
    } else if ( timer <= 60)
    {
      ...
      // ロゴのアルファ値を少しずつ上げていく。
      draw.setAlpha( R.drawable.title, (float)timer / 60.0f );
      // ゆっくりとロゴが上に動く。
      draw.drawTextureScalingC( R.drawable.title, 0.0f, 0.0f, w, h / 2.0f,
          draw.getWidth() / 2.0f, draw.getHeight() / 2.0f - timer * dh / 4.0f / 60.0f,
          dw, dh / 2.0f );
    } else
    {
      // 待機画面。
